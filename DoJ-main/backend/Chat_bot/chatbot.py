...
def response(prompt):
    ...
    try:
        # Check if vector_index is defined and available
        if vector_index:
            ...
            # Combine results from QA chain and SerpApi
            if qa_answer.lower() == "i don't know":
                # Fallback to SerpApi answers if QA chain answer is "I don't know"
                answer = serpapi_answers
            else:
                # answer = f"QA Chain answer: {qa_answer} SerpApi answers: {serpapi_answers}"
                answer=serpapi_answers
